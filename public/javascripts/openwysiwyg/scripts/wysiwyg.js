...
var WYSIWYG = {
	...
	Settings: function() {
		...
		// Images Directory
		this.ImagesDir = "/javascripts/openwysiwyg/images/";
		
		// Popups Directory
		this.PopupsDir = "/javascripts/openwysiwyg/popups/";
		
		// CSS Directory File
		this.CSSFile = "/javascripts/openwysiwyg/styles/wysiwyg.css";
	...
	"createlink":     ['CreateLink',           'Create Link',         	'insert_hyperlink.gif',   'insert_hyperlink_on.gif'],
        "inserttag":      ['InsertTag',            'Insert Tag',                'insert_tag.gif',         'insert_tag_on.gif'],
	...
	attachAll: function(settings) {
		...
		for(var i=0;i<areas.length;i++) {
			...
			this.setSettings(id, settings);
			WYSIWYG_Core.includeCSS(this.config[id].CSSFile);
                        WYSIWYG_Core.includeCSS(this.config[id].IncludeCSS);
	...
	_generate: function(n, settings) {
		...
		// Set default font style
                WYSIWYG_Core.includeCSS(this.config[n].IncludeCSS);
		WYSIWYG_Core.setAttribute(doc.body, "style", this.config[n].DefaultStyle);
	...
	execCommand: function(n, cmd, value) {
		...
		var textModeCommands = new Array("ViewText", "Print", "InsertTag");
		...
		switch(cmd) {
			...
			// InsertTag
			case "InsertTag": 
				window.open('/admin/pages/tags?wysiwyg=' + n, 'popup', 'location=0,status=0,scrollbars=1,resizable=0,width=600,height=330,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
