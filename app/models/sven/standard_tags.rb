module Sven	
	module StandardTags
		...
		desc %{
			...
			*Usage:*
			<pre><code><s:page>...</s:page></code></pre>
		}
		tag 'page' do |tag|
		...
		desc %{
			Writes full title for page, used in <head />
		}
		...
		desc %{
			Generates link to stylesheet
		}
		tag 'stylesheets' do |tag|
			@stylesheets = ""
			Css.all( :conditions => { :status_id => Status[:published].id } ).each do |style|
				@stylesheets << "<link href=\"#{style.url}\" media=\"screen\" rel=\"stylesheet\" type=\"text/css\" />"
			end
...
desc %{ 
    Renders the main content of a page. If is set @length@ paremeter, it returns @lenght@ characters of content.
    
    *Usage:*
    <pre><code><r:content [length=X] /></code></pre>
  }
		tag 'content' do |tag|
			length = (tag.attr['length'] || tag.locals.page.body.length).to_i
			tag.locals.page.body.to(length)
    end

		desc %{
			Writes template content
		}
		tag 'template_content' do |tag|
			@tmpl = tag.locals.page.template
			@tmpl = ContentTemplate.find(:first) if @tmpl.blank?
		...
		tag 'menu' do |tag|
			generate_menu tag.globals.page.url
    end
		...
		tag 'languages' do |tag|
			...
			File.open("#{RAILS_ROOT}/app/views/site/languages.html.erb", "r") do |erb|
				ERB.new(erb.read).result(binding)
			end
    end
  ...
  tag 'url' do |tag|
    relative_url_for(tag.locals.page.url, @request)
  end

	desc %{
		Renders the url for page, specified in @page@ attribute.
	}
	tag 'url_for' do |tag|
		page = tag.attr['page']
		raise TagError.new('\'page\' attribute must be specified') unless page

		Page.find_by_slug(page).url
  end
  ...
  tag "attempted_url" do
	...
	desc %{
		Evalueates content of this tag
	}
	tag 'eval' do |tag|
		contents = tag.expand
		eval contents
  ...
  desc %{
    Returns found pages as childs of search page
  }
  tag 'found' do |tag|
    contents = SearchPage.args[:found]

    if contents
      contents.each do |found|
        tag.locals.page = found
        tag.expand
      end
    end
  ...
  private
		...
		def children_find_options(tag)
			...
			order_string = (attr[:order_by] || 'title').strip
			order = (attr[:order] || 'asc').strip
			options[:order] = "#{order_string} #{order}"

			status = (attr[:status] || 'published').downcase
			unless status == 'all'
				stat = Status[status]
				unless stat.nil?
					options[:conditions] = ["(status_id = ?)", stat.id]
				else
		...
		def annex_find_options(tag)
			...
			order_string = (attr[:order_by] || 'filename').strip
			order = (attr[:order] || 'asc').strip
			options[:order] = "#{order_string} #{order}"
		...
		def generate_menu(menuselect)
			@page ||= Page.find_by_slug '/'
			@whole_menu ||= "<div id=\"menu\"><ul><li>"
			@whole_menu << erb_me!
			@whole_menu << "</li>"
		...
		def page_children(parent = 1)
			childs = ""

			for @page in Page.find(:all, :conditions => ["class_name = 'Page' AND status_id = ? AND parent_id = ? AND language = ?", Status[:published].id, parent, Sven::LanguageHolder.language], :order => 'lft')
				childs << "<li"
				#unless @page.children.blank?
				#	childs << " onmouseover=\"slideMe('page-#{@page.id}-child', 'in')\""
				#	childs << " onmouseout=\"slideMe('page-#{@page.id}-child', 'out')\""
        #end
				childs << ">"
				childs << erb_me!
				unless @page.children.find_by_status_id(Status[:published].id).blank?
					childs << "<ul id=\"page-#{@page.id}-child\">#{page_children(@page.id)}</ul>"
				end
				childs << "</li>"
			end
			childs
		...
		def erb_me!
			"<span><a href=\"#{@page.url}\">#{@page.title}</a></span>"
			#File.open("#{RAILS_ROOT}/app/views/site/menu.html.erb", "r") { |erb| return ERB.new(erb.read).result(binding) }
    end
	end
end